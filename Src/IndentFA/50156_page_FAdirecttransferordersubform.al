page 50156 "FA Transfer Order Subform"
{
    AutoSplitKey = true;
    Caption = 'Lines';
    DelayedInsert = true;
        ...
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Item No."; Rec."Item No.")
                {
                    ...
                    //  TableRelation = Item where(IsFixedAssetItem = const(true));
                    ...
                    begin
                        // if Item.get('9999999') then
                        //     if Page.RunModal(Page::"Item List", Item) = Action::OK then;
                        // rec."Item No." := '9999999'
                        TempItem.SetFilter(TempItem.IsFixedAssetItem, '%1', true);
                        IF PAGE.RUNMODAL(0, TempItem) = ACTION::LookupOK THEN begin
                            Rec.Validate(Rec."Item No.", TempItem."No.");
                        end;
                    end;
                }
                ...
                field(FixedAssetNo; FixedAssetNo)
                {
                    ApplicationArea = all;
                    trigger OnValidate()
                    var
                    ...
                    begin
                        if FARec2.get(rec.FixedAssetNo) then begin
                            rec."GST Credit" := FARec2."GST Credit";
                            rec."GST Group Code" := FARec2."GST Group Code";
                            rec."HSN/SAC Code" := FARec2."HSN/SAC Code";
                            rec.Modify();
                        end;

                    end;
                }
                field("Parent Fixed Asset"; "Parent Fixed Asset")
                {
                    ApplicationArea = all;
                }
    ...
    end;//need to check 240224

    trigger OnNewRecord(BelowxRec: Boolean)
    begin
        Clear(ShortcutDimCode);
        // rec.Validate("Item No.", '9999999');
    end;
