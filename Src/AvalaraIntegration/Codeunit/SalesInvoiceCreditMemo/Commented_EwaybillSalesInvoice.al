/*
codeunit 50148 "EwayBillSalesinvoice"
{
    ...
    procedure SetSalesInvHeader(SalesHeaderBuff: Record "Sales Invoice Header")
    begin
        SalesInvoiceHeader := SalesHeaderBuff;
        IsTransfer := true;
    end;
    ...
    /*
        local procedure RunSalesInvoiceEwayBill()
        begin
        ...
        begin
            ...
            JTransporter.Add('name', ShippingAgent.Name);
            JTransporter.Add('transporterId', ShippingAgent."GST Registration No.");
            if SalesInvoiceHeader."Mode of Transport" = '1' then
                JTransporter.add('modeOfTransport', 'Road');

            JTransporter.add('documentNumber', SalesInvoiceHeader."No.");
            JTransporter.add('documentDate', SalesInvoiceHeader."Posting Date");
            dist1 := SalesInvoiceHeader."Distance (Km)";
            Evaluate(dist3, format(dist1));
            JTransporter.add('distance', dist3);


            JTransporter.add('vehicleNo', SalesInvoiceHeader."Vehicle No.");
            ...
            JTransporter.add('vehicleType', VehType)
        ...
        begin
            if TempCustomer.get(SalesInvoiceHeader."Sell-to Customer No.") then;
        ...
        begin
            SalesInvoiceNumber := SalesInvoiceHeader."No.";
            type1 := 'AR-IN';
            Transferdate := Format(SalesInvoiceHeader."Posting Date");
            Pos := ''; //place of supply
            OtherAmount := 0;
            ...
            JObject.Add('Number', SalesInvoiceNumber);
        ...
        begin
            if TempLocation.get(SalesInvoiceHeader."Location Code") then;
            // if tempState.Get(TempLocation."State Code") then;


            city1 := TempLocation.City;
            IF tempState.Get(SalesInvoiceHeader."Location State Code") then;
        ...
        var
            ...
            TempLocation: Record Location;
            temppostcode: Record "Post Code";
            tempSIL: Record "Sales Invoice Line";

        begin
            tempSIL.Reset();
            tempSIL.SetRange("Document No.", SalesInvoiceHeader."No.");
            tempSIL.SetFilter("GST Place of Supply", '<>%1', tempSIL."GST Place of Supply"::" ");
            IF tempSIL.FindFirst() then begin
                If tempSIL."GST Place of Supply" = tempSIL."GST Place of Supply"::"Bill-to Address" then begin
                    city2 := SalesInvoiceHeader."Bill-to City"; //'test';//
                    if tempState.get(SalesInvoiceHeader."GST Bill-to State Code") then;
                    state2 := tempState."State Code (GST Reg. No.)"; //'03';//
                    Line2 := SalesInvoiceHeader."Bill-to Address"; //'teststes test';
                    Country2 := SalesInvoiceHeader."Bill-to County";//'IN';//
                    Postcode2 := SalesInvoiceHeader."Bill-to Post Code" //'143001';//
                end;

                If tempSIL."GST Place of Supply" = tempSIL."GST Place of Supply"::"Ship-to Address" then begin
                    city2 := SalesInvoiceHeader."Ship-to City"; //'test';//
                    if tempState.get(SalesInvoiceHeader."GST Ship-to State Code") then;
                    state2 := tempState."State Code (GST Reg. No.)"; //'03';//
                    Line2 := SalesInvoiceHeader."Ship-to Address"; //'teststes test';
                    Country2 := SalesInvoiceHeader."Ship-to County";//'IN';//
                    Postcode2 := SalesInvoiceHeader."Ship-to Post Code"; //'143001';//
                end;
            ...
            if TempSalesLine.FindSet() then begin
                ...
                repeat
                    ...
                    WriteItem(
                      ...
                      TempSalesLine."HSN/SAC Code", CopyStr(TempSalesLine."Unit of Measure Code", 1, 3),
        ...
        begin
            ...
            JItem.Add('HSNCode', '84099191'); //HSNCode
            JItem.Add('UnitQuantityCode', 'PCS');//Unit
                        ...
                        if JobjToken.IsObject then begin
                            ...
                            if JobjectResponse.Get('EwayBillOutput', EwayJsonToken) then begin
                                ...
                                if EwayJsonToken.IsObject then begin
                                    ...
                                    tempSIV."E-Way Bill No." := EwayJsonToken.AsValue().AsText();
                                    tempSIV.Modify(true);
                            ...
                            if JobjectResponse.Get('Errors', errorDetailsJsonToken) then begin
                                ...
                                for j := 0 to JobjErrorarr.Count() - 1 do begin
                                    ...
                                    if errorDetailsJsonToken.IsObject then begin
                                        errorDetailsJsonObject := errorDetailsJsonToken.AsObject();
                                        errorDetailsJsonObject.Get('Message', errorDetailsJsonToken);
                                        TempMsg := errorDetailsJsonToken.AsValue().AsText();
                                        errorDetailsJsonObject.Get('Explanation', errorDetailsJsonToken);
                                        IF (errorDetailsJsonToken.AsValue().AsText() <> '') then begin
                                            Error(TempMsg + ' ' + errorDetailsJsonToken.AsValue().AsText());
                                        end;
                                    end;
                                end;
                            end;
