...
codeunit 50124 DebitNoteEway
{
    ...
    begin
        ...
        RunDebitNoteEwayBill();

        if DocumentNo <> '' then
            ExportAsJsonDebitNote(DocumentNo)
        else
    ...
    procedure SetPurchaseDebitNote(PurchaseHeaderBuff: Record "Purch. Cr. Memo Hdr.")
    begin
        PurchCrMemoHeader := PurchaseHeaderBuff;
    ...
    local procedure RunDebitNoteEwayBill()
    begin
    ...
    begin
        ...
        JTransporter.add('documentNumber', PurchCrMemoHeader."No.");
        JTransporter.add('documentDate', PurchCrMemoHeader."Posting Date");
        dist1 := PurchCrMemoHeader."Distance (Km)";
        Evaluate(dist3, format(dist1));
        JTransporter.add('distance', dist3);


        JTransporter.add('vehicleNo', PurchCrMemoHeader."Vehicle No.");
        JTransporter.add('vehicleType', format(PurchCrMemoHeader."Vehicle Type"));
    ...
    var
        ...
        name1: Text[50];
        UIN: text[50];
        TempVendor: Record Location;

    begin
        if TempVendor.get(PurchCrMemoHeader."Pay-to Vendor No.") then;


        GSTNumber1 := TempVendor."GST Registration No.";//'03EHFPS5910D3A1';// //'03EHFPS5910D3Z0';
        name1 := TempVendor.Name;//'test customer hardcoded';//
        UIN := '';
    ...
    begin
        PurchaseInvoiceNumber := PurchCrMemoHeader."No.";
        type1 := 'AR-IN';
        Transferdate := Format(PurchCrMemoHeader."Posting Date");
        Pos := ''; //place of supply
        OtherAmount := 0;
        ...
        JObject.Add('Number', PurchaseInvoiceNumber);
    ...
    begin
        if TempLocation.get(PurchCrMemoHeader."Location Code") then;
        // if tempState.Get(TempLocation."State Code") then;


        city1 := TempLocation.City;
        IF tempState.Get(PurchCrMemoHeader."Location State Code") then;
    ...
    var
        ...
        TempLocation: Record Location;
        temppostcode: Record "Post Code";
        tempCML: Record "Purch. Cr. Memo Line";
    begin


        city2 := PurchCrMemoHeader."Pay-to City"; //'test';//
        if tempState.get(PurchCrMemoHeader."Location State Code") then;

        state2 := tempState."State Code (GST Reg. No.)"; //'03';//
        Line2 := PurchCrMemoHeader."Pay-to Address"; //'teststes test';
        Country2 := PurchCrMemoHeader."Pay-to County";//'IN';//
        Postcode2 := PurchCrMemoHeader."Pay-to Post Code";//'143001';//         
        ...
        if SalesInvoiceHeader."Ship-to Code" <> '' then begin
            ...
            state2 := SalesInvoiceHeader."GST Ship-to State Code"; //'03';//
    ...
    local procedure ExportAsJsonDebitNote(FileName: Text[20])
    var
        ...
        tempPurchCrHdr: Record "Purch. Cr. Memo Hdr.";
        TempMsg: Text;
                    ...
                    if JobjToken.IsObject then begin
                        ...
                        if JobjectResponse.Get('EwayBillOutput', EwayJsonToken) then begin
                            ...
                            if EwayJsonToken.IsObject then begin
                                ...
                                tempPurchCrHdr.Reset();
                                tempPurchCrHdr.SetRange("No.", PurchCrMemoHeader."No.");
                                IF tempPurchCrHdr.FindFirst() then;

                                EwayJsonObject := EwayJsonToken.AsObject();
                                EwayJsonObject.Get('Eway_bill_id', EwayJsonToken);
                                tempPurchCrHdr."E-Way Bill No." := EwayJsonToken.AsValue().AsText();
                                tempPurchCrHdr.Modify(true);
